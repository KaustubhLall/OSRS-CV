...
class MacroGUI:
    ...
    def create_widgets(self):
        ...
        self.speculative_mode_check = tk.Checkbutton(param_frame, text="Speculative Mode",
                                                     variable=self.speculative_mode)
        ...
        tk.Label(param_frame, text="Interface Wait (s):").grid(row=row, column=0, sticky='e')
        ...
        dark_mode_check = tk.Checkbutton(param_frame, text="Dark Mode", variable=self.dark_mode,
                                         command=self.toggle_dark_mode)
        dark_mode_check.grid(row=row, column=0, columnspan=2)
        row += 1

        # Button to configure positions
        tk.Button(param_frame, text="Configure Positions", command=self.configure_positions).grid(row=row, column=0,
                                                                                                  columnspan=2, pady=5)
    ...
    def configure_positions(self):
        ...
        for key, value in self.positions.items():
            ...
            # Add Set button
            set_button = tk.Button(pos_window, text="Set",
