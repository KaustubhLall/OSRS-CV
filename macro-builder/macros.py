        ...
        try:
            ...
            while infinite_loop or self.current_loop_index < total_loops:
                ...
                if self.is_loop_macro and not infinite_loop:
                    ...
                    ) / self.current_loop_index
                    estimated_time_remaining = loops_remaining * average_time_per_loop
                    self.app.after(
                        0, self.app.update_eta_tracker, estimated_time_remaining
                    )
            ...
            self.app.after(
                0, self.app.log_macro_execution, self.name, total_log, total_time
            )
            self.current_loop_index = 0
        ...
        for action in actions:
            ...
            if action_type == "press_panel_key":
                ...
                if use_saved_target:
                    ...
                    if self.is_dose_macro:
                        pos_index = local_state["current_position_index"] % len(
                            positions
                        )
                ...
                if isinstance(pos[0], (list, tuple)):
                    ...
                    for p in pos:
                        pyautogui.moveTo(
                            p[0], p[1], duration=self.app.mouse_move_duration
                        )
                        pyautogui.click()
                        log.append(
                    ...
                    # Single position
                    pyautogui.moveTo(
                        pos[0], pos[1], duration=self.app.mouse_move_duration
                    )
                    pyautogui.click()
                    log.append(
                ...
                if self.is_dose_macro:
                    ...
                    if local_state["call_count"] % self.dose_count == 0:
                        local_state["current_position_index"] = (
                            local_state["current_position_index"] + 1
                        ) % self.get_total_positions()
                        log.append(
                            f"Cycled to next position index: {local_state['current_position_index']}"
                        )
                ...
                if isinstance(duration, str):
                    duration = self.app.wait_times.get(
                        duration, self.app.action_registration_time()
                    )
                time.sleep(duration)
                log.append(
                ...
                sub_macro = next(
                    (m for m in self.app.macro_list_data if m.name == macro_name), None
                )
                if sub_macro:
                    if sub_macro == self:
                        ...
                        if getattr(sub_macro, "local_state", None) is None:
                            sub_macro.local_state = {
                                "call_count": sub_macro.call_count,
                                "current_position_index": sub_macro.current_position_index,
                            }

                        sub_macro.run_actions(
                            sub_macro.actions,
                            original_position,
                            log,
                            context,
...
class MacroApp(tk.Tk):
    def __init__(self):
        ...
        self.action_registration_time_min = self.config.get(
            "action_registration_time_min", 0.02
        )
        self.action_registration_time_max = self.config.get(
        ...
        self.inventory_key = self.config.get("specific_panel_keys", {}).get(
            "Inventory", "w"
        )
        ...
        self.task_executor = TaskExecutor(
            self.task_queue, self.task_execution_delay, self
        )
    ...
    def toggle_macro_enabled_state(self, macro):
        ...
        macro_config = next(
            (m for m in self.config["macros"] if m["name"] == macro.name), None
        )
        if macro_config:
    ...
    def config_load(self):
        try:
            ...
            self.config = {
                ...
                "panel_key": "q",
                "specific_panel_keys": {"Inventory": "w", "Prayer": "e", "Spells": "r"},
    ...
    def on_macro_click(self, event):
        ...
        if column == "#5":  # Assuming 'Reset' is the 5th column
            ...
            macro_name = values[0]
            macro = next(
                (m for m in self.macro_list_data if m.name == macro_name), None
            )
    ...
    def create_status_indicator(self):
        ...
        self.status_indicator_canvas = tk.Canvas(
            status_frame, height=50, bg="grey", highlightthickness=0
        )
    ...
    def create_settings_tab(self):
        ...
        settings_canvas = tk.Canvas(
            self.settings_frame, borderwidth=0, background="#f0f0f0"
        )
        scrollbar = ttk.Scrollbar(
            self.settings_frame, orient="vertical", command=settings_canvas.yview
        )
        ...
        timing_frame = ttk.LabelFrame(
            scrollable_frame, text="Timing Settings", padding=(10, 10)
        )
        timing_frame.grid(row=0, column=0, padx=5, pady=5, sticky="nsew")

        # Interface Switch Time
        ttk.Label(timing_frame, text="Interface Switch Time (s):").grid(
        ...
        self.interface_switch_time_entry.grid(
            row=0, column=1, padx=5, pady=5, sticky="w"
        )

        # Action Registration Time Min
        ttk.Label(timing_frame, text="Action Registration Time Min (s):").grid(
        ...
        ttk.Label(timing_frame, text="Action Registration Time Max (s):").grid(
        ...
        # --- Panel Keys ---
        panel_keys_frame = ttk.LabelFrame(
            scrollable_frame, text="Panel Keys", padding=(10, 10)
        )
        panel_keys_frame.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")

        # Panel Key
        ttk.Label(panel_keys_frame, text="Panel Key:").grid(
            row=0, column=0, sticky="e", padx=5, pady=5
        )
        ...
        # Inventory Panel Key
        ttk.Label(panel_keys_frame, text="Inventory Panel Key:").grid(
            row=1, column=0, sticky="e", padx=5, pady=5
        )
        ...
        # Prayer Panel Key
        ttk.Label(panel_keys_frame, text="Prayer Panel Key:").grid(
            row=2, column=0, sticky="e", padx=5, pady=5
        )
        ...
        # Spells Panel Key
        ttk.Label(panel_keys_frame, text="Spells Panel Key:").grid(
            row=3, column=0, sticky="e", padx=5, pady=5
        )
        ...
        # --- Wait Times ---
        wait_times_frame = ttk.LabelFrame(
            scrollable_frame, text="Wait Times", padding=(10, 10)
        )
        ...
        ttk.Label(wait_times_frame, text="Duration (s)").grid(
            row=0, column=1, padx=5, pady=5, sticky="w"
        )
        ...
        add_wait_btn = ttk.Button(
            btn_frame, text="Add Wait Time", command=self.add_wait_time
        )
        add_wait_btn.pack(side="left", padx=5)

        delete_wait_btn = ttk.Button(
            btn_frame, text="Delete Selected", command=self.delete_wait_time
        )
        delete_wait_btn.pack(side="left", padx=5)

        # --- Task Execution Delay ---
        task_delay_frame = ttk.LabelFrame(
            scrollable_frame, text="Task Execution Delay", padding=(10, 10)
        )
        task_delay_frame.grid(
            row=1, column=0, columnspan=3, padx=5, pady=5, sticky="ew"
        )

        task_delay_frame.columnconfigure(1, weight=1)

        ttk.Label(task_delay_frame, text="Delay Between Tasks (s):").grid(
            row=0, column=0, sticky="e", padx=5, pady=5
        )
        ...
        self.task_execution_delay_entry.grid(
            row=0, column=1, padx=5, pady=5, sticky="w"
        )

        # --- Save Settings Button ---
        save_settings_btn = ttk.Button(
            scrollable_frame, text="Save Settings", command=self.save_settings
        )
    ...
    def create_macros_tab(self):
        ...
        for macro in self.config["macros"]:
            ...
            call_count = (
                macro.get("call_count", 0) if macro.get("is_dose_macro", False) else ""
            )
        ...
        edit_macro_btn = ttk.Button(
            btn_frame, text="Edit Macro", command=self.edit_macro
        )
        edit_macro_btn.pack(side="left", padx=5)

        copy_macro_btn = ttk.Button(
            btn_frame, text="Copy Macro", command=self.copy_macro
        )
        copy_macro_btn.pack(side="left", padx=5)

        del_macro_btn = ttk.Button(
            btn_frame, text="Delete Macro", command=self.delete_macro
        )
        del_macro_btn.pack(side="left", padx=5)

        # Add "Reset All" Button
        reset_all_btn = ttk.Button(
            btn_frame, text="Reset All", command=self.reset_all_macros
        )
...
class MacroEditor(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Name
        ttk.Label(self, text="Macro Name:").grid(
        ...
        # Hotkey
        ttk.Label(self, text="Hotkey:").grid(
        ...
        self.disabled_check = ttk.Checkbutton(
            self, text="Disable Macro", variable=self.disabled_var
        )
        self.disabled_check.grid(
        ...
        self.is_dose_macro_check = ttk.Checkbutton(
            self,
            text="Enable Dose-Counting",
            variable=self.is_dose_macro_var,
            command=self.toggle_dose_count_entry,
        )
        self.is_dose_macro_check.grid(
            row=4, column=0, columnspan=3, padx=10, pady=5, sticky="w"
        )

        # Number of Doses Entry (only visible if dose-counting is enabled)
        ttk.Label(self, text="Number of Doses:").grid(
        ...
        self.is_loop_macro_check = ttk.Checkbutton(
            self,
            text="Enable Looping",
            variable=self.is_loop_macro_var,
            command=self.toggle_loop_count_entry,
        )
        self.is_loop_macro_check.grid(
            row=6, column=0, columnspan=3, padx=10, pady=5, sticky="w"
        )

        # Number of Loops Entry
        ttk.Label(self, text="Number of Loops (-1 for infinite):").grid(
        ...
        self.schedule_check.grid(
            row=8, column=0, columnspan=3, padx=10, pady=5, sticky="w"
        )

        ttk.Label(self, text="Schedule Delay (e.g., 2m5s):").grid(
        ...
        ttk.Label(self, text="Repeat Interval (e.g., 1h30m):").grid(
        ...
        add_action_btn = ttk.Button(
            btn_frame, text="Add Action", command=self.add_action
        )
        add_action_btn.pack(side="top", padx=5, pady=2)

        edit_action_btn = ttk.Button(
            btn_frame, text="Edit Action", command=self.edit_action
        )
        edit_action_btn.pack(side="top", padx=5, pady=2)

        copy_action_btn = ttk.Button(
            btn_frame, text="Copy Action", command=self.copy_action
        )
        copy_action_btn.pack(side="top", padx=5, pady=2)

        del_action_btn = ttk.Button(
            btn_frame, text="Delete Action", command=self.delete_action
        )
        ...
        move_down_btn = ttk.Button(
            btn_frame, text="Move Down", command=self.move_action_down
        )
    ...
    def get_action_description(self, action):
        ...
        if action_type == "press_panel_key":
            ...
            if action.get("use_saved_target", False):
                ...
                modifiers = action.get("modifiers", [])
                description = (
                    f"Click at Positions {positions} with modifiers {modifiers}"
                )
        elif action_type == "return_mouse":
    ...
    def load_macro_data(self):
        ...
        if self.macro_config.get("schedule", None):
            ...
            self.repeat_interval_entry.config(state="normal")
            self.repeat_interval_entry.insert(
                0, self.macro_config.get("repeat_interval", "")
            )
        else:
    ...
    def move_action_down(self):
        ...
        if (
            selected
            and self.actions_tree.index(selected[0]) < len(self.actions_list) - 1
        ):
            index = self.actions_tree.index(selected[0])
            self.actions_list[index + 1], self.actions_list[index] = (
                self.actions_list[index],
    ...
    def save_macro(self):
        ...
        if is_dose_macro:
            ...
            if not dose_count_str.isdigit() or int(dose_count_str) <= 0:
                messagebox.showerror(
                    "Invalid Input", "Number of Doses must be a positive integer."
                )
                return
        ...
        if self.schedule_var.get():
            ...
            if not schedule:
                messagebox.showerror(
                    "Invalid Input", "Please enter a valid schedule delay."
                )
                return
            parsed_schedule = parse_time(schedule)
            if parsed_schedule is None:
                messagebox.showerror(
                    "Invalid Format", "Schedule delay format is invalid."
                )
                return
            ...
            if repeat_interval:
                ...
                if parsed_repeat is None:
                    messagebox.showerror(
                        "Invalid Format", "Repeat interval format is invalid."
                    )
                    return
        ...
        if is_dose_macro:
            ...
            new_macro["call_count"] = (
                self.macro_config.get("call_count", 0) if self.macro_config else 0
            )
            new_macro["current_position_index"] = (
                self.macro_config.get("current_position_index", 0)
                if self.macro_config
                else 0
            )
        else:
        ...
        if self.macro_config and not self.is_copy:
            ...
            call_count = (
                new_macro["call_count"] if new_macro.get("is_dose_macro", False) else ""
            )
...
class ActionEditor(tk.Toplevel):
    ...
    def create_widgets(self):
        # Action Type
        ttk.Label(self, text="Action Type:").grid(
        ...
        self.action_type_menu = ttk.OptionMenu(
            self,
            self.selected_action_type,
            self.action_types[0],
            *self.action_types,
            command=self.update_action_fields,
        )
        ...
        # Annotation
        ttk.Label(self, text="Annotation:").grid(
            row=2, column=0, padx=10, pady=5, sticky="e"
        )
        self.annotation_entry = ttk.Entry(self, width=50)
        self.annotation_entry.grid(
            row=2, column=1, columnspan=2, padx=10, pady=5, sticky="w"
        )
        ...
        if self.action:
            # Load action data
            self.selected_action_type.set(
                self.action.get("type").replace("_", " ").title()
            )
    ...
    def update_action_fields(self, *args):
        ...
        if action_type == "Press Panel Key":
            ...
            ttk.Label(self.params_frame, text="Panel:").grid(
                row=0, column=0, padx=5, pady=5
            )
            self.panel_options = ["Inventory", "Prayer", "Spells", "Custom"]
            self.selected_panel = tk.StringVar()
            self.selected_panel.set("Inventory")
            self.panel_menu = ttk.OptionMenu(
                self.params_frame,
                self.selected_panel,
                self.panel_options[0],
                *self.panel_options,
                command=self.toggle_custom_panel_entry,
            )
            ...
            self.use_saved_target_check = ttk.Checkbutton(
                self.params_frame,
                text="Use Saved Target Position",
                variable=self.use_saved_target_var,
            )
            self.use_saved_target_check.grid(
                row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w"
            )

            ttk.Label(self.params_frame, text="Positions:").grid(
            ...
            add_pos_btn = ttk.Button(
                btn_frame, text="Add Positions", command=self.add_click_positions
            )
            add_pos_btn.pack(side="top", padx=5, pady=2)

            del_pos_btn = ttk.Button(
                btn_frame, text="Delete Position", command=self.delete_click_position
            )
            del_pos_btn.pack(side="top", padx=5, pady=2)

            # Modifiers
            ttk.Label(self.params_frame, text="Modifiers:").grid(
            ...
            self.shift_check = ttk.Checkbutton(
                self.params_frame, text="Shift", variable=self.shift_var
            )
            self.shift_check.grid(row=2, column=1, padx=5, pady=5, sticky="w")
            self.ctrl_check = ttk.Checkbutton(
                self.params_frame, text="Ctrl", variable=self.ctrl_var
            )
            self.ctrl_check.grid(row=2, column=1, padx=5, pady=5)
            self.alt_check = ttk.Checkbutton(
                self.params_frame, text="Alt", variable=self.alt_var
            )
            ...
            self.click_after_return_check = ttk.Checkbutton(
                self.params_frame,
                text="Click After Return",
                variable=self.click_after_return_var,
            )
            self.click_after_return_check.grid(
                row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w"
            )
        elif action_type == "Wait":
            self.use_custom_wait_var = tk.BooleanVar(value=False)
            self.use_custom_wait_check = ttk.Checkbutton(
                self.params_frame,
                text="Use Custom Wait Time",
                variable=self.use_custom_wait_var,
                command=self.toggle_wait_duration_entry,
            )
            self.use_custom_wait_check.grid(
                row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w"
            )
            ...
            self.wait_time_menu = ttk.OptionMenu(
                self.params_frame,
                self.selected_wait_time,
            ...
            ttk.Label(self.params_frame, text="Duration (s):").grid(
            ...
            ttk.Label(self.params_frame, text="Select Macro:").grid(
                row=0, column=0, padx=5, pady=5
            )
            self.macro_names = [
                macro["name"]
                for macro in self.parent.parent.config["macros"]
                if macro["name"] != self.parent.name_entry.get()
            ]
            self.selected_macro_name = tk.StringVar()
            self.selected_macro_name.set(
                self.macro_names[0] if self.macro_names else ""
            )
            self.macro_menu = ttk.OptionMenu(
                self.params_frame,
